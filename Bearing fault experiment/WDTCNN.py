...
from keras.layers import Input, Activation, Dense, Flatten, AveragePooling1D, Conv1D,Dropout
...
# Full WDTCNN model function
def full_model_WDTCNN():
    ...
    for train, test in kf.split(data.X_train, data.y_train['label']): # Must specify y StratifiedKFold for 
        ...
        convnet.add(Dense(data.nclasses, activation = 'sigmoid'))
        ...
        # Fit the model
        convnet.fit(x_train, y_train,validation_data = (x_test,y_test), epochs = 500, batch_size = 32, verbose=1, 
...
# Initialize the full_model_WDTCNN function
oos_test_y = []
oos_test_prob = []
aggregated_score = 0
...
oos_test_prob, oos_test_y, aggregated_score, aggregated_test_score, runtime, earlystop, oos_test_activations, oos_test_y = full_model_WDTCNN()
